...
std::vector<double> CG::maximize(GaussianProcess* gp, size_t n, bool verbose)
{
	using namespace std;
	cout << "using cg optimizer" << endl;
	...
	*/
	...
	cout << "gp->log_likelihood() = " << gp->log_likelihood() << endl;
	cout << "-gp->log_likelihood() = " << -gp->log_likelihood() << endl;
	...
	log_likelihood_curve.push_back(f0);
	if(verbose) cout << "1 " << f0 <<  endl;
	...
	for (unsigned int i = 0; i < n; ++i)
	{
		...
		while(1)											//keep extrapolating until necessary
		{
			...
			while( !success && M>0)
			{
				...
				log_likelihood_curve.push_back(f3);
				if(verbose) cout << "2 "<< f3 <<  endl;
	...
	cout << "ttttttt" << "log_likelihood_curve.size() = " << log_likelihood_curve.size() << endl;
